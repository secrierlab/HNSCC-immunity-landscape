...
# TLS signature genes:
# from https://www.nature.com/articles/s41586-019-1914-8, fig 1b:
TLSa <- c("CCL19","CCL21","CXCL13","CCR7",
          "SELL","LAMP3","CXCR4","CD86","BCL6")
TLSb <- c("CCL2","CCL3","CCL4","CCL5","CCL8","CCL18",
          "CCL19","CCL21","CXCL9","CXCL10","CXCL11","CXCL13")
TLSchemokine <- c("CCL19","CCL21","CXCL12","CXCL13")
# from https://www.nature.com/articles/s41586-019-1914-8, fig 3a:
TLSc <- c("CD79B","CD1D","CCR6","LAT","SKAP1",
...
setdiff(TLSc, colnames(dat.merged))
# the melanoma specific list is missing 6 out of 9 genes
# -> cannot evaluate TLSc as is, but could use some of the genes as markers
TLSmel_genes <- intersect(TLSc, colnames(dat.merged))
...
pdf("plots2021/TLSsignatures_plusAux.pdf",w=14,h=8)
...
pdf("plots2021/TLSsignatures_allsigs.pdf",w=8,h=5)
...
pdf("plots2021/TLSfigure_IHCvariables.boxplot.pdf",w=14,h=4)
ggboxplot(df.melt3, 
          x = "ImmunityGroup", 
          y = "logval",
          palette = c("#4A6FA5", "#DCD6F7","#FFE89F",
...
pdf("plots2021/CD83FOXP3expr_Signatures.pdf",w=8,h=5)
...
pdf("plots2021/IHC_Hscores.boxplot.pdf",w=10,h=8)
ggboxplot(df.melt6, 
          x = "ImmunityGroup", 
          y = "value",
          palette = c("#4A6FA5", "#DCD6F7","#FFE89F",
...
pdf("plots2021/IHC_totalStain.boxplot.pdf",w=12,h=6)
ggboxplot(df.melt7, 
          x = "ImmunityGroup", 
          y = "value",
          palette = c("#4A6FA5", "#DCD6F7","#FFE89F",
...
pdf("plots2021/comparingHPVstatus.pdf",w=6,h=5)
...
pdf("plots2021/comparingPDL1status.pdf",w=8,h=5)
...
df.meltSmokers <- melt(dat.merged[,c("Smoking.Category",
                                  "observed_richness","TLSchemokine")])

my_comparisons4 <- list(c("Never","Past"),c("Past","Present"),
                        c("Never","Present"))
pdf("plots2021/comparingSmokerstatus.pdf",w=6,h=6)
...
pdf("plots2021/TMBvsprodclon.pdf")
...
pdf("plots2021/TMBvsobsrichness.pdf")
ggscatter(dat.merged[which(dat.merged$TMB<250),],
...
pdf("plots2021/TMBvsobsrichness.log10.pdf")
...
pdf("plots2021/Packyearsvsobsrichness.pdf")
...
pdf("plots2021/PackyearsvsPRODclon.pdf")
ggscatter(dat.merged, 
...
pdf("plots2021/Packsperdaysvsobsrichness.pdf")
ggscatter(dat.merged, 
...
pdf("plots2021/PacksperdayvsPRODclon.pdf")
ggscatter(dat.merged, 
...
pdf("plots2021/TLSvsobsrichness.pdf")
...
pdf("plots2021/agevsobsrichness.pdf")
...
pdf("plots2021/TLSvsprodclon.pdf")
...
pdf("plots2021/TLSvsprodclon_byImmg.withlines.pdf")
ggscatter(dat.merged[which(!is.na(dat.merged$Productive.Clonality)),], 
          x = "TLSsignature", y = "Productive.Clonality",
          #add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE,
  ...
  geom_vline(xintercept=8.7, linetype="dashed", color = "grey")
# median is 8.17
dev.off()
...
dat.merged1[which((dat.merged1$TLSsignature>8.7)&
                   (dat.merged1$Productive.Clonality>0.085)),]$TLSPCgroup <-"Exhausted"
dat.merged1[which((dat.merged1$TLSsignature<=8.7)&
...
pdf("plots2021/TLSvsobsrich_byImmg.pdf")
...
pdf("plots2021/TLSvsobsrich_byImmg.log10.pdf")
ggscatter(dat.merged2, 
  ...
  geom_vline(xintercept=8.75, linetype="dashed", color = "grey")
dev.off()

pdf("plots2021/TLSvsobsrich_byImmg.log10.plushist.pdf",w=10,h=10)
...
dat.merged2[which((dat.merged2$TLSsignature>8.7)&
             (dat.merged2$observed_richness_log>2.8)),]$TLSORgroup <-"Exhausted"
dat.merged2[which((dat.merged2$TLSsignature<=8.7)&
                   (dat.merged2$observed_richness_log<=2.8)),]$TLSORgroup <-"Absent"
dat.merged2[which((dat.merged2$TLSsignature<=8.7)&
                   (dat.merged2$observed_richness_log>2.8)),]$TLSORgroup <-"Mixed"
dat.merged2[which((dat.merged2$TLSsignature>8.7)&
                   (dat.merged2$observed_richness_log<=2.8)),]$TLSORgroup <-"DCrich"

pdf("plots2021/TLSvsprodclon_byPDL1.pdf")
ggscatter(dat.merged[which(!is.na(dat.merged$Productive.Clonality)),], 
          x = "TLSsignature", y = "Productive.Clonality",
          #add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE,
...
pdf("plots2021/survival.byGroupComplexTLS.pdf",onefile = FALSE,w=20)
ggsurvplot(fit2,          
...
dat.merged$ProdClonBinary <- sapply(dat.merged$productive_clonality,
                                      function(x) ifelse(is.na(x),NA,ifelse(x>0.10,"high","low")))
dat.merged$TLSBinary <- sapply(dat.merged$TLSsignature,
                                    function(x) ifelse(is.na(x),NA,ifelse(x>9,"high","low")))
...
# Drawing survival curves. Change color, linetype by strata, risk.table color by strata
pdf("plots2021/survival.byTLSORgroup.pdf",onefile = FALSE,w=6,h=10)
ggsurvplot(fit4,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/survival.byTLSORgroup.binary.pdf",onefile = FALSE,w=6,h=10)
ggsurvplot(fit4sub,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/pfs.byTLSORgroupBinary.pdf",onefile = FALSE,w=6,h=6)
...
pdf("plots2021/pfs.byTLSORgroup.pdf",onefile = FALSE,w=6)
ggsurvplot(fit4pfs,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/survival.byTLSPCgroup.pdf",onefile = FALSE,w=6)
ggsurvplot(fit5,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/pfs.byTLSPCgroup.pdf",onefile = FALSE,w=6)
ggsurvplot(fit5pfs,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/survival.byTLSPCgroupBinary.pdf",onefile = FALSE,w=6)
ggsurvplot(fit6,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("plots2021/pfs.byTLSPCgroupBinary.pdf",onefile = FALSE,w=6)
ggsurvplot(fit6pfs,          
           pval = TRUE, #conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
